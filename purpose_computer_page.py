...
class purpose_computers_page(tk.Frame):

    def __init__(self, purpose_name = None ,customer_name = None, customer_username = None,
                        customer_Id = None,master = None):
        tk.Frame.__init__(self, master)
        self.purpose = purpose_name

        self.master.title( f"{self.purpose} Computers Page" )
    ...
    def create_widgets(self):
        ...
        if self.Customer_Name == None:
            ...
            self.LabelTitle = tk.ttk.Label( self.top, 
                                            text = f"Recommended {self.purpose} Computers", 
        ...
        for i in range(10): # Only the top 10 sold computers will be shown
            computer_name = df_purpose.iloc[i]['Name']
            computer_type = df_purpose.iloc[i]['Type']
            ...
            self.Buttons[i].place( relx = 0 + plus_relx , rely = 0.5 + plus_rely,
                                   relwidth = 0.1595, relheight = 0.051)
    ...
    def command_computers(self, computer_name, computer_type):
        ...
        if self.Customer_Name == None: # We are in the guess account 
            generalized_item.generalized_item( coming_from = "purpose_computer_page",
